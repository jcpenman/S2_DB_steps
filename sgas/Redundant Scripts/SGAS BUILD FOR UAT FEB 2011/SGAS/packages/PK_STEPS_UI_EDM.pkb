...
******************************************************************************/
   ...
   BEGIN
      OPEN e_cursor FOR
         SELECT   unique_id, source_db, annot, attachment_type_code,
                  batch_id, batch_type_code, document_name,
                  document_type_code, document_type_count, envelope_id,
                  object_id, raw_data_id, req_original, rescan, rescan_req,
                  rescan_request_id, scan_date, session_code, upload_date,
                  viewed_doc
             FROM (SELECT ROWNUM || 'S' AS unique_id, 'S' AS source_db, ei.annot,
                          ei.attachment_type_code, ei.batch_id,
                    ...
                    WHERE ei.stud_ref_no = stud_ref_no_in
                      ...
                      AND ei.rescan != 'Y'
                   UNION
                   SELECT ROWNUM || 'G' AS unique_id, 'G' AS source_db, gei.annot,
                          gei.attachment_type_code, gei.batch_id,
