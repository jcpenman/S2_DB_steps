...
******************************************************************************/
  ...
  BEGIN
    SELECT NVL( SUM(a.tuition_fees_saas), 0 )
      INTO p_tuition_fees_saas
      FROM vu_award_notification_award a
  ...
  BEGIN
    ...
    FROM AWARD a, vu_award_notification_stud b
    WHERE a.STUD_CRSE_YEAR_ID = p_stud_crse_year_id
  ...
  BEGIN
    SELECT NVL( SUM(a.loan_for_fees), 0 )
  ...
  BEGIN
    OPEN c_students FOR
        SELECT s.stud_crse_year_id                          AS stud_crse_year_id,
               ...
               s.spouse_cont - resid_par_cont               AS partner_contrib,
               sgas.pk_award_notification.get_student_contibution(stud_crse_year_id)   AS student_contrib,
               NVL(s.snb_overpayment,0)                     AS overpayment_recovered,
               ...
               sgas.pk_award_notification.get_remark1(s.stud_crse_year_id)             AS rem1, 
               sgas.pk_award_notification.get_remark2(s.stud_crse_year_id)             AS rem2,  
               s.rem3,  s.rem4,
  ...
  BEGIN
    OPEN c_stud_awards FOR
        SELECT sa.award_id                  AS award_id,  
               ...
               sa.award_type_desc           AS award_type_desc,
               sa.amount                    AS amount,
                CASE
                     WHEN sa.loan_non_loan_fee = 'Loan'
                        THEN 'Y'
                     WHEN sa.loan_non_loan_fee = 'fee'
                        THEN 'F'
                     ELSE 'N'
                  END AS is_loan,
                sa.show_on_an_payments
          FROM VU_AWARD_NOTIFICATION_AWARD sa
...
SELECT   to_date(payment_date,'DD/MM/YYYY') AS payment_date, SUM (pay_amount) pay_amount, pay_location
    FROM (SELECT   payment_date, pay_amount, pay_location
              FROM (SELECT 1 AS ord, (apu.payment_due_date) AS payment_date,
                      ...
                      FROM vu_award_notification_unpaid apu
                    ...
                    SELECT 0 AS ord, (app.dpp_payment_date) AS payment_date,
                      ...
                      FROM vu_award_notification_paid app
