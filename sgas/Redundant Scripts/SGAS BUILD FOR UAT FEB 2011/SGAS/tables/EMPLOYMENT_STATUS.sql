...
-- Description: Table holding all EMPLOYMENT_STATUSs for SGAS
...
ALTER TABLE sgas.employment_status
 DROP PRIMARY KEY CASCADE
/
DROP TABLE sgas.employment_status CASCADE CONSTRAINTS PURGE
/
--
-- EMPLOYMENT_STATUS  (Table) 
--
CREATE TABLE sgas.employment_status
(
  employment_status_id      NUMBER(10)       NOT NULL,
...
COMMENT ON COLUMN sgas.employment_status.employment_status_id IS 'Unique EMPLOYMENT_STATUS reference number';

COMMENT ON COLUMN sgas.employment_status.legacy_code IS 'Legacy system GRASS/WEB character';

COMMENT ON COLUMN sgas.employment_status.descript IS 'Description of data item';

COMMENT ON COLUMN sgas.employment_status.last_updated_by IS 'Audit information of last user to update record';

COMMENT ON COLUMN sgas.employment_status.last_updated_on IS 'Audit information of last date record was updated';


CREATE UNIQUE INDEX employment_status_pk ON sgas.employment_status
...
CREATE OR REPLACE TRIGGER SGAS.emp_stat_iud
   AFTER INSERT OR DELETE OR UPDATE OF EMPLOYMENT_STATUS_id,
                                       legacy_id,
                                       legacy_code,
                                       descript,
                                       last_updated_by
   ON SGAS.EMPLOYMENT_STATUS    FOR EACH ROW
DECLARE
...
ALTER TABLE sgas.employment_status ADD (
...
-- 
DROP PUBLIC SYNONYM employment_status
/
CREATE PUBLIC SYNONYM employment_status FOR sgas.employment_status
/
DROP SEQUENCE sgas.employment_status_id_seq
/
--
-- EMPLOYMENT_STATUS_ID_seq  (Sequence) 
--
CREATE SEQUENCE sgas.employment_status_id_seq
  START WITH 1
  ...
  NOORDER
/
CREATE OR REPLACE TRIGGER sgas.trig_employment_status_seq
   BEFORE INSERT
   ON sgas.employment_status
   FOR EACH ROW
BEGIN
   SELECT employment_status_id_seq.NEXTVAL
     INTO :NEW.employment_status_id
     FROM DUAL;
END;                                                                        
...
INSERT INTO employment_status
     ...
     VALUES (0, 'U', 'Unemployed'
            );

INSERT INTO employment_status
            (legacy_id, legacy_code, descript
            )
     VALUES (0, 'S', 'Self-employed'
            );
