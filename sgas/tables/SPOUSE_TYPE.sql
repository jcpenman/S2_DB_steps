-- SPOUSE_TYPE.sql
-- Description: Table holding all SPOUSE_TYPEs for SGAS
...
ALTER TABLE sgas.spouse_type
 DROP PRIMARY KEY CASCADE
/
DROP TABLE sgas.spouse_type CASCADE CONSTRAINTS
/
--
-- SPOUSE_TYPE  (Table) 
--
CREATE TABLE sgas.spouse_type
(
  spouse_type_id      NUMBER(10)       NOT NULL,
...
COMMENT ON COLUMN sgas.spouse_type.spouse_type_id IS 'Unique SPOUSE_TYPE reference number';

COMMENT ON COLUMN sgas.spouse_type.legacy_code IS 'Legacy system GRASS/WEB character';

COMMENT ON COLUMN sgas.spouse_type.descript IS 'Description of data item';

COMMENT ON COLUMN sgas.spouse_type.last_updated_by IS 'Audit information of last user to update record';

COMMENT ON COLUMN sgas.spouse_type.last_updated_on IS 'Audit information of last date record was updated';


CREATE UNIQUE INDEX spouse_type_pk ON sgas.spouse_type
(spouse_type_id)
LOGGING
TABLESPACE users
PCTFREE    10
...
CREATE OR REPLACE TRIGGER SGAS.spo_type_iud
   AFTER INSERT OR DELETE OR UPDATE OF SPOUSE_TYPE_id,
                                       legacy_id,
                                       legacy_code,
                                       descript,
                                       last_updated_by
   ON SGAS.SPOUSE_TYPE    FOR EACH ROW
DECLARE
...
ALTER TABLE sgas.spouse_type ADD (
  CONSTRAINT spouse_type_pk
 PRIMARY KEY
 (spouse_type_id)
    USING INDEX
    TABLESPACE users
...
-- 
DROP PUBLIC SYNONYM spouse_type
/
CREATE PUBLIC SYNONYM spouse_type FOR sgas.spouse_type
/
DROP SEQUENCE sgas.spouse_type_id_seq
/
--
-- SPOUSE_TYPE_ID_seq  (Sequence) 
--
CREATE SEQUENCE sgas.spouse_type_id_seq
  START WITH 1
  ...
  NOORDER
/
CREATE OR REPLACE TRIGGER sgas.trig_spouse_type_seq
   BEFORE INSERT
   ON sgas.spouse_type
   FOR EACH ROW
BEGIN
   SELECT spouse_type_id_seq.NEXTVAL
     INTO :NEW.spouse_type_id
     FROM DUAL;
END;                                                                        
...
INSERT INTO spouse_type
     ...
     VALUES (28, 'O', 'HUSBAND, WIFE OR PARTNER (GRASS)'
            );
