-- OTHER_LOAN_TYPE.sql
-- Description: Table holding all OTHER_LOAN_TYPEs for SGAS
...
ALTER TABLE sgas.other_loan_type
 DROP PRIMARY KEY CASCADE
/
DROP TABLE sgas.other_loan_type CASCADE CONSTRAINTS PURGE
/
--
-- OTHER_LOAN_TYPE  (Table) 
--
CREATE TABLE sgas.other_loan_type
(
  other_loan_type_id      NUMBER(10)       NOT NULL,
...
COMMENT ON COLUMN sgas.other_loan_type.other_loan_type_id IS 'Unique OTHER_LOAN_TYPE reference number';

COMMENT ON COLUMN sgas.other_loan_type.legacy_code IS 'Legacy system GRASS/WEB character';

COMMENT ON COLUMN sgas.other_loan_type.descript IS 'Description of data item';

COMMENT ON COLUMN sgas.other_loan_type.last_updated_by IS 'Audit information of last user to update record';

COMMENT ON COLUMN sgas.other_loan_type.last_updated_on IS 'Audit information of last date record was updated';


CREATE UNIQUE INDEX other_loan_type_pk ON sgas.other_loan_type
(other_loan_type_id)
LOGGING
TABLESPACE users
PCTFREE    10
...
ALTER TABLE sgas.other_loan_type ADD (
  CONSTRAINT other_loan_type_pk
 PRIMARY KEY
 (other_loan_type_id)
    USING INDEX
    TABLESPACE users
...
-- 
DROP PUBLIC SYNONYM other_loan_type
/
CREATE PUBLIC SYNONYM other_loan_type FOR sgas.other_loan_type
/
DROP SEQUENCE sgas.other_loan_type_id_seq
/
--
-- OTHER_LOAN_TYPE_ID_seq  (Sequence) 
--
CREATE SEQUENCE sgas.other_loan_type_id_seq
  START WITH 1
  ...
  NOORDER
/
CREATE OR REPLACE TRIGGER sgas.trig_other_loan_type_seq
   BEFORE INSERT
   ON sgas.other_loan_type
   FOR EACH ROW
BEGIN
   SELECT other_loan_type_id_seq.NEXTVAL
     INTO :NEW.other_loan_type_id
     FROM DUAL;
END;                                                                        
...
INSERT INTO other_loan_type
            (legacy_id, legacy_code, descript
            )
     VALUES (0, 'A', 'Non Loan Bearing'
            );
INSERT INTO other_loan_type
            (legacy_id, legacy_code, descript
            )
     VALUES (0, 'B', 'Loan Restricted - Eligibility'
            );
INSERT INTO other_loan_type
            (legacy_id, legacy_code, descript
            )
     VALUES (0, 'C', 'Loan Restricted - Application'
            );
INSERT INTO other_loan_type
            (legacy_id, legacy_code, descript
            )
     VALUES (0, 'D', 'No loan Claimed'
            );
INSERT INTO other_loan_type
            (legacy_id, legacy_code, descript
            )
     VALUES (0, 'E', 'Loan Amount Claimed'
            );
INSERT INTO other_loan_type
            (legacy_id, legacy_code, descript
            )
     VALUES (0, 'F', 'Maximum Loan Claimed'
            );
