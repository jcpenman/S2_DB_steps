-- DUP_BANK_REASON.sql
-- Description: Table holding all DUP_BANK_REASONs for SGAS
...
ALTER TABLE sgas.DUP_BANK_REASON
 DROP PRIMARY KEY CASCADE
/
DROP TABLE sgas.DUP_BANK_REASON CASCADE CONSTRAINTS
/
--
-- DUP_BANK_REASON  (Table) 
--
CREATE TABLE sgas.DUP_BANK_REASON
(
  DUP_BANK_REASON_id      NUMBER(10)       NOT NULL,
...
COMMENT ON COLUMN sgas.DUP_BANK_REASON.DUP_BANK_REASON_id IS 'Unique DUP_BANK_REASON reference number';

COMMENT ON COLUMN sgas.DUP_BANK_REASON.legacy_code IS 'Legacy system GRASS/WEB character';

COMMENT ON COLUMN sgas.DUP_BANK_REASON.descript IS 'Description of data item';

COMMENT ON COLUMN sgas.DUP_BANK_REASON.last_updated_by IS 'Audit information of last user to update record';

COMMENT ON COLUMN sgas.DUP_BANK_REASON.last_updated_on IS 'Audit information of last date record was updated';


CREATE UNIQUE INDEX DUP_BANK_REASON_pk ON sgas.DUP_BANK_REASON
(DUP_BANK_REASON_id)
LOGGING
TABLESPACE users
PCTFREE    10
...
CREATE OR REPLACE TRIGGER SGAS.DUP_BANK_REA_iud
   AFTER INSERT OR DELETE OR UPDATE OF DUP_BANK_REASON_id,
                                       legacy_id,
                                       legacy_code,
                                       descript,
                                       last_updated_by
   ON SGAS.DUP_BANK_REASON    FOR EACH ROW
DECLARE
...
ALTER TABLE sgas.DUP_BANK_REASON ADD (
  CONSTRAINT DUP_BANK_REASON_pk
 PRIMARY KEY
 (DUP_BANK_REASON_id)
    USING INDEX
    TABLESPACE users
...
-- 
DROP PUBLIC SYNONYM DUP_BANK_REASON
/
CREATE PUBLIC SYNONYM DUP_BANK_REASON FOR sgas.DUP_BANK_REASON
/
DROP SEQUENCE sgas.DUP_BANK_REASON_id_seq
/
--
-- DUP_BANK_REASON_ID_seq  (Sequence) 
--
CREATE SEQUENCE sgas.DUP_BANK_REASON_id_seq
  START WITH 1
  ...
  NOORDER
/
CREATE OR REPLACE TRIGGER sgas.trig_DUP_BANK_REASON_seq
   BEFORE INSERT
   ON SGAS.DUP_BANK_REASON
   FOR EACH ROW
BEGIN
   SELECT DUP_BANK_REASON_id_seq.NEXTVAL
     INTO :NEW.DUP_BANK_REASON_id
     FROM DUAL;
END;                                                                        
...
-- INSERT DATA
--
INSERT INTO DUP_BANK_REASON
            (legacy_id, legacy_code, descript
            )
     VALUES (0, 'JA', 'Joint Account'
            );
...
INSERT INTO DUP_BANK_REASON
     ...
     VALUES (0, 'RU', 'Account Number Reused After 5 Years'
            );
