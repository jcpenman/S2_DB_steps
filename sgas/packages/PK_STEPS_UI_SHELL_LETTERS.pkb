...
******************************************************************************/
   ...
   PROCEDURE getshellcatlist (
      io_cursor       IN OUT          shellcat_cursor,
      error_boolean   OUT NOCOPY      VARCHAR2,
      ERROR_TEXT      OUT NOCOPY      VARCHAR2      
   )
   IS 
        sc_cursor shellcat_cursor;
   BEGIN
        
        OPEN sc_cursor FOR
            SELECT stl.doc_name, stl.doc_desc, stc.cat_description, stc.cat_code FROM 
            std_letters stl, std_letters_categories stc
            WHERE stl.cat_code = stc.cat_code
            ORDER BY stl.doc_name;
