   ...
   *********************************************/
   ...
   BEGIN
      DBMS_OUTPUT.PUT_LINE (
            'Copying session '
         || TO_CHAR (new_session_code - 1)
         || ' to session '
         || TO_CHAR (new_session_code)
         || '.');

      -- First Check whether session code is in correct format
      IF LENGTH (new_session_code) > 4
      THEN
      ...
      SELECT COUNT (*)
       ...
       WHERE item_name = 'FEES_CUTOFF_' || new_session_code;

      IF v_count = 0
      THEN
         DBMS_OUTPUT.PUT_LINE (
            'No CONFIG_DATA record for FEES_CUTOFF_' || new_session_code);
         RETURN 'Processing Terminated with errors';
      ELSE
         ...
         SELECT COUNT (*)
           INTO v_count
           FROM scoap_data@GRASS
          WHERE item_name LIKE 'FEES_CUTOFF_%' || new_session_code;

         IF v_count = 0
         THEN
            DBMS_OUTPUT.PUT_LINE (
                  'No SCOAP_DATA records for fees cutoff dates in session '
               || new_session_code);
            RETURN 'Processing Terminated with errors';
         ELSE
            -- Continue processing
            v_result := find_crse_rollover (new_session_code);
         END IF;

         DBMS_OUTPUT.PUT_LINE (
            v_session_count || ' crse_session records processed ');
         DBMS_OUTPUT.PUT_LINE (
            v_year_count || ' crse_year records processed ');
   ...
   FUNCTION find_crse_rollover (new_session_code IN NUMBER)
      RETURN VARCHAR2
   IS
      CURSOR get_crse_session_dets
      IS
           -- Get all crse_sesion_id where no later sessions than p_session exist
           SELECT crse_session_id, inst_code, crse_code
             FROM CRSE_SESSION cs, CRSE c
            WHERE     session_code = new_session_code - 1
                  AND c.crse_id = cs.crse_id
                  AND c.crse_id = cs.crse_id
                  AND cs.crse_id NOT IN
                         (SELECT crse_id
                            FROM CRSE_SESSION
                           WHERE session_code >= new_session_code)
   ...
   BEGIN
      ...
      LOOP
         ...
         FETCH get_crse_session_dets
         INTO v_crse_session_id, v_inst_code, v_crse_code;

         EXIT WHEN get_crse_session_dets%NOTFOUND;

         IF v_crse_session_id IS NULL
         THEN
            -- No course session details to roll forwards - exit
            DBMS_OUTPUT.PUT_LINE (
               'No course session details to roll forwards. Processing terminated');
            RETURN ('OK');
         END IF;
         ...
         IF v_max_crse_year_id != v_min_crse_year_id
         THEN
            -- get a count of all non final year students for crse_session
            v_activity :=
               'getting a count of all non final year students for crse_session';

            SELECT COUNT (*)
              INTO v_count
              FROM STUD_CRSE_YEAR
             WHERE     crse_year_id IN
                          (SELECT crse_year_id
                             FROM CRSE_YEAR
                            WHERE     crse_session_id = v_crse_session_id
         ...
         ELSE
            ...
            -- get a count of all non final year students for crse_session
            v_activity :=
               'getting a count of all students for crse_session on 1 year course';
         ...
         THEN
            ...
            v_result := do_crse_rollover (new_session_code, v_crse_session_id);
         END IF;
      END LOOP;
   ...
   BEGIN
      ...
      INSERT INTO CRSE_SESSION (crse_session_id,
                                crse_id,
                                session_code,
                                tuition_fee_type_code,
                                max_duration,
      ...
      BEGIN
         ...
         LOOP
            ...
            IF     v_cutoff_item_name NOT LIKE 'FEES_CUTOFF_DEFAULT%'
               AND v_cutoff_item_name IS NOT NULL
            THEN
               ...
               DBMS_OUTPUT.PUT_LINE (
                  ...
                  || ', Year '
                  || v_crse_year_no
                  || ' allocated default cutoff date as previous session used '
                  || v_cutoff_type);
            END IF;

            -- set all item names to be the default one where one existed lastcrse_year
            IF v_cutoff_item_name IS NOT NULL
            THEN
               v_cutoff_item_name :=
                  'FEES_CUTOFF_DEFAULT_' || new_session_code;

               --
               SELECT description, TO_DATE (cval, 'DD/MM/YYYY')
                 INTO v_cutoff_type, v_cutoff_date
                 FROM SCOAP_DATA@GRASS
                WHERE item_name = v_cutoff_item_name;
            --
            END IF;

            SELECT crsy_crse_year_id_seq.NEXTVAL
      ...
      EXCEPTION
         ...
         THEN
            ...
            DBMS_OUTPUT.PUT_LINE (
               ...
               || ' '
               || v_sql_message
               || ' occurred while copying crse_year details for Inst'
               || v_inst_code
               || ', Crse '
               || v_crse_code);
            ROLLBACK;
   ...
   EXCEPTION
      ...
      THEN
         ...
         DBMS_OUTPUT.PUT_LINE (
            ...
            || ' '
            || v_sql_message
            || ' occurred while copying crse_session details for Inst'
            || v_inst_code
            || ', Crse '
            || v_crse_code);
         ROLLBACK;
