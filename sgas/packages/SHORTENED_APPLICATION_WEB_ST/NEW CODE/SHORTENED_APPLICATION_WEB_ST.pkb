...
FUNCTION get_shwap(p_object_id IN EDM_IMAGES.object_id%TYPE,
		            ...
		            p_web_user_id IN COMPLETED_SHWAP.web_user_id%TYPE,
                    p_top_option IN COMPLETED_SHWAP.top_option%TYPE) -- JM RFC 248
RETURN varchar2 IS
...
--  null recs
    ...
    m_edm_rec.rd_top_option := p_top_option ;
    -----ADDED FOR EDM DECOMISSIONING------
    m_edm_rec.rd_object_id := p_object_id;
    ---------------------------------------
    v_process := TRUE;
--
--COMMENTED OUT FOR EDM DECOMISSIONING-------
/*
...
--
CURSOR	fetch_rec IS
SELECT	stud_ref_no, session_code, object_id, web_submitted_date,web_user_id, top_option
...
--
   v_return := get_shwap(rec.object_id, rec.stud_ref_no, rec.web_submitted_date, rec.session_code, rec.web_user_id, rec.top_option);
