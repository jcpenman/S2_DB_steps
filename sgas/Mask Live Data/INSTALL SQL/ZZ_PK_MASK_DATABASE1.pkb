CREATE OR REPLACE PACKAGE BODY SGAS.zz_pk_mask_database
AS
 /******************************************************************************
...
*/
    ...
    BEGIN
        ...
        LOOP
         ...
         THEN
            v_update_sql := v_update_sql||' ZZ_PK_MASK_DATABASE.CREATE_NEW_KEY(:1, :2, :3, :4, T.'||get_key.column_name||')';
         END IF;
         IF (get_key.table_name = 'EMPLOYEE')
         THEN
            v_update_sql := v_update_sql || ' WHERE T.USERNAME LIKE '''||'U%''';
         END IF;
