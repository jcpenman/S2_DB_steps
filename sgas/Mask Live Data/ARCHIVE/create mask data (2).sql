...
BEGIN
   ...
   FOR GET_REC IN c_nominee
   LOOP
      UPDATE zz_mask_nominee S
         SET S.NOMINEE_ID = GET_REC.NOMINEE_ID
       WHERE S.RECORD_NO = (SELECT MIN (S1.RECORD_NO)
                              FROM zz_mask_nominee S1
                             WHERE S1.NOMINEE_ID IS NULL);
   END LOOP;
...
BEGIN
   ...
   LOOP
      UPDATE zz_mask_stud_dependant_name S
         SET S.std_id = GET_REC.std_id
       WHERE S.RECORD_NO = (SELECT MIN (S1.RECORD_NO)
                              FROM zz_mask_stud_dependant_name S1
                             WHERE S1.std_id IS NULL);
   END LOOP;
...
BEGIN
   ...
   FOR GET_REC IN c_benefactor
   LOOP
      UPDATE zz_mask_benefactor S
         SET S.BEN_ID = GET_REC.BEN_ID
       WHERE S.RECORD_NO = (SELECT MIN (S1.RECORD_NO)
                              FROM zz_mask_benefactor S1
                             WHERE S1.BEN_ID IS NULL);
   END LOOP;
