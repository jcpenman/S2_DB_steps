CREATE OR REPLACE PACKAGE BODY ILA500.pk_pop_aud
IS
-- DESCRIPTION
-- ===========
...
-- 11.11.2008           A Bowman    003    Added procedures for DEBT_STATUS table
--
-- Configuration Management:
-- $HeadURL: svn://192.168.186.3/projects/sgas/tradev/svnRepositories/steps/trunk/Database/steps/ILA500/Packages/PK_POP_AUD.pkb $
-- $Author: $
...
PROCEDURE ins_config_data_aud (
  ...
  PROCEDURE ins_debt_status_aud (
   ...
   BEGIN
      IF (p_action = 'D')
      THEN
         SELECT debt_status_aud_id_seq.NEXTVAL
           INTO pl_aud_id
           FROM DUAL;
...
-- Set the logging name on the respective audit table with the authenticated login name of the application user.
   ...
   BEGIN
      UPDATE ila500_edm_images_aud
         ...
         AND aud_date BETWEEN SYSDATE - (1 / 24) AND SYSDATE;
   END;
      PROCEDURE set_uid4audit_debt_status_aud (
      p_user_id         VARCHAR2,
      p_primary_key   VARCHAR2
   )
   AS
