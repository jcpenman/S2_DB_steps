-- VU_SDS.sql
-- Description: dbView holding all learner contact detail and application status change data for ILA500 SDS interface for 2009 session year.
...
DROP VIEW ILA500.VU_SDS2009;

/* Formatted on 2010/07/30 14:19 (Formatter Plus v4.8.8) */
CREATE OR REPLACE FORCE VIEW ila500.vu_sds2009 (ila_ref_no,
                                                title,
                                                title_change_date,
                                                last_name,
                                                surname_change_date,
        ...
        AND lap.learner_application_id =
               (SELECT MAX (learner_application_id)
                 ...
                 WHERE lap2.learner_id = l.learner_id
                   AND lap2.session_year = 2009)
        AND ti.title_id = l.title_id
        AND lap.session_year = 2009
   ORDER BY l.learner_id ASC;




GRANT DELETE, INSERT, SELECT, UPDATE, ON COMMIT REFRESH, QUERY REWRITE ON  VU_SDS2009 TO EDM_USER;
